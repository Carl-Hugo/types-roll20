declare global {
    ...
    type UnderscoreVariants<T extends string> = T extends `_${infer Value}` ? Value | T : T
    type PropertyOrDefault<Object, Key, Default = unknown> = Key extends keyof Object ? Object[Key] : Default
    ...
    type PathFix<T> = T extends Path ? { path: Path['_path'] } : {}
    ...
    type ObjectFromChangeEvent<Event extends AllChangeEvents> = Event extends `change:${infer Type & (ObjectTypes | SubTypes)}`
        ? Type
        : Event extends `change:${infer Type & (ObjectTypes | SubTypes)}`
        ? Type
        : never

    type BuiltInEffect = 'beam' | 'bomb' | 'breath' | 'bubbling' | 'burn' | 'burst' | 'explode' | 'glow' | 'missile' | 'nova' | 'splatter'
    type EffectColor = 'acid' | 'blood' | 'charm' | 'death' | 'fire' | 'frost' | 'holy' | 'magic' | 'slime' | 'smoke' | 'water'
    type EffectType<T extends string> = T extends `-${infer S}`
    ...
    type AnyRoll20Object = { [K in keyof ObjectTypeMap]: Roll20Object<ObjectTypeMap[K]> }[ObjectTypes]
    type AnyObject = ObjectTypeMap[ObjectTypes]
    type FindObjType<T> = T extends Record<'_type', infer X & ObjectTypes>
    ...
    type NonRandomCustomFXDefinition = {
        angle: number,
        duration: number
        emissionRate: number,
        gravity: number,
    ...
    type CustomFXDefinition = NonRandomCustomFXDefinition
        & { [T in keyof NonRandomCustomFXDefinition as `${T}Random`]: NonRandomCustomFXDefinition[T] }
        & Record<'onDeath', string>
    type Point = { x: number, y: number }

    type ObjectCreateProperties<T extends ObjectTypes> = { [Key in CreatableProp<T>]: CreatableValue<ObjectTypeMap[T], Key> }
    type CreatableProp<T extends ObjectTypes> = WritableKeys<ObjectTypeMap[T]> | `${ForeignKey<string & keyof ObjectTypeMap[T]>}id`
    type ForeignKey<T extends string> = T extends '_id' ? never : T extends `_${infer Obj}id` ? Obj : never
    type CreatableValue<T, K> = K extends keyof T
        ? T[K]
    ...
    type SubTypes = 'card' | 'token'

    type ObjectTypes = keyof ObjectTypeMap
    // have to use extract so that TS knows that CreatableObjectTypes is a subset of ObjectTypes
    type CreatableObjectTypes = Extract<ObjectTypes, 'graphic' | 'text' | 'path' | 'character' | 'ability' | 'attribute' | 'handout' | 'rollabletable' | 'tableitem' | 'macro'>
    const state: any

    type Roll20Message = Roll20GeneralMessage | Roll20RollResultMessage | Roll20WhisperMessage | Roll20WhisperMessage | Roll20ApiMessage
    type Roll20MessageBase = {
        readonly who: string,
    ...
    type Roll20GeneralMessage = Roll20MessageBase & Readonly<{ type: 'general' | 'emote' | 'desc', }>
    type Roll20RollResultMessage = Roll20MessageBase & Readonly<{ origroll: string, type: 'rollresult' | 'gmrollresult' }>
    type Roll20WhisperMessage = Roll20MessageBase & Readonly<{ target: string, target_name: string, type: 'whisper' }>
    type Roll20ApiMessage = Roll20MessageBase & Readonly<{ selected: MessageSelectType[], type: 'api' }>

    type MessageSelectType = {
        readonly _id: string,
    ...
    type Roll20Object<T extends ObjectTypeMap[ObjectTypes]> = {
        readonly id: string,
